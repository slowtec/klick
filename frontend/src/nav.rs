        ...
        <div class= move || if mobile_menu_is_open.get() { "sm:hidden" } else { "hidden" } >
          ...
          { move ||
            if let Some(user_info) = user_info.get() {
              view! { <MobileUserMenu user_info /> }.into_view()
            } else {
...
#[component]
fn MobileUserMenu(user_info: UserInfo) -> impl IntoView {
    view! {
      <div class="border-t border-gray-200 pb-3 pt-4">
        ...
        <div class="mt-3 space-y-1">
          <a href="#" class="block px-4 py-2 text-base font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-800">"Logout"</a>
        </div>
      </div>
    }
}
