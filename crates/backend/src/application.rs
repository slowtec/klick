...
impl AppState {
    pub fn create_user(&mut self, credentials: Credentials) -> Result<(), CreateUserError> {
    ...
    pub fn login(&mut self, email: EmailAddress, password: &str) -> Result<Uuid, LoginError> {
    ...
    pub fn authorize_user(&self, token: &str) -> Result<CurrentUser, AuthError> {
        token
            .parse::<Uuid>()
